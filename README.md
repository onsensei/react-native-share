...
1. `npm install onsensei-react-native-social-share --save`
2. `react-native link`
...
3. Go to `node_modules` ➜ `onsensei-react-native-social-share` ➜ `ios` and add `RNShare.xcodeproj`
...
3. Append the following lines to `android/settings.gradle`:
  	```
  	include ':onsensei-react-native-social-share'
  	project(':onsensei-react-native-social-share').projectDir = new File(rootProject.projectDir, 	'../node_modules/onsensei-react-native-social-share/android')
  	```
4. Insert the following lines inside the dependencies block in
   `android/app/build.gradle`:

    ```
      compile project(':onsensei-react-native-social-share')
    ```
5. Follow this
...
6. Edit your `MainApplication` class to implement `ShareApplication`
  ...
  - For example: Replace the `com.example.yourappidhere` below with the
    ...
    properly](https://github.com/onsensei/react-native-social-share/issues/200#issuecomment-361938532).
...
1. `npm install onsensei-react-native-social-share --save`
2. In Visual Studio add the `RNShare.sln` in `node_modules/onsensei-react-native-social-share/windows/RNShare.sln` folder to their solution, reference from their app.
...
import Share, {ShareSheet, Button} from 'onsensei-react-native-social-share';
